...
class Administracion extends BaseController
{
	...
	protected $nombre_modulo = 'administracion';
		...
		if (is_login()) {
			$script = '<script>
                $(document).ready(function(){
                    cargar_inicio_modulo("administracion");
                });
            </script>';
	...
	public function usuarios()
	{
		...
		if (!validar_permiso('administracion', 'usuarios', 'consultar')) {
		...
		switch (getsegment(3)) {
			case 'listado':
				...
				{
					var_dump('Entro a permiso');
					$usuarios = $usuariosModel->getAll();
				}

				else
				{
					$usuarios = $usuariosModel->where('id_organizacion', getSession('id_organizacion'))->obtener('all');
				}
				...
				$data_tabla = array(
					...
					'nombre_tabla' => 'listado_administracion_usuarios',
					'dataTable' => array(
						'usuarios' => $usuarios
					)
				);
				...
				{
					$datos_usuario = array(
						'organizaciones' => $organizacionesModel->obtener('organizacion'),
						'roles' => $rolesModel->obtener('organizacion'),
					);
				}
				...
				$data_form = array(
					...
					'nombre_form' => 'frm_administracion_usuarios'
				);
			...
			default:
				$data = array(
					'script' => "<script>
									$(document).ready(function(){
										cargar_listado('administracion', 'usuarios', '" . baseUrl('administracion/usuarios/listado') . "');
									});
								</script>"
				);
	...
	public function roles()
	{
		...
		if (!validar_permiso('administracion', 'roles', 'consultar')) {
			$error = $this->object_error(403, 'No tiene permiso para realizar esta acción');
		...
		switch (getsegment(3)) {
			case 'listado':
				...
				$data_tabla = array(
					'nombreTable' => $nombreTabla,
					'nombre_tabla' => 'listado_administracion_roles',
					'dataTable' => array(
						'roles' => $roles
					)
				);
				...
				$dataForm = array(
					...
					'tipos_roles'=> $tiposOrganizacionModel->getAll(),
				);

				$data_form = array(
					'dataForm' => $dataForm,
					'nombreForm' => $nombreForm,
					'nombre_form' => 'frm_administracion_roles'
				);
			...
			default:
				$data = array(
					'script' => "<script>
										$(document).ready(function(){
											cargar_listado('administracion', 'roles', '" . baseUrl('administracion/roles/listado') . "');
										});
									</script>"
				);
		...
		if (is_login()) {
			if (getSegment(3) == 'listado') {
				...
				$data = array(
					'script' => '<script>
						$(document).ready(function(){
							cargar_listado("administracion", "auditorias", "' . baseUrl('administracion/auditorias/listado') . '");
						});
					</script>'
				);
	...
	public function errores()
	{
		if (getSegment(3) == 'listado') {
			...
			$data = array(
				'script' => '<script>
						$(document).ready(function(){
							cargar_listado("administracion", "errores", "' . baseUrl('administracion/errores/listado') . '");
						});
					</script>'
			);
			...
			if (!is_null($objeto) && in_array($objeto, $this->objetos)) {
				switch ($objeto) {
					case 'usuarios':
						...
						if (validar_permiso('administracion', 'usuarios', 'insertar')) {
	...
	public function enviar_contrasenia()
	{
		if (getSegment(3)) {
			//Validar permiso
			if (!validar_permiso('administracion', 'usuarios', 'modificar'))
				return json_encode(array(
					'error' => 'No tiene permisos para realizar esta acción.',
				));
						...
						switch ($id) {
							case 'perfil':
								...
								if ($model->update($data, $id)) {
									...
									setSession('genero', post('genero'));
									setSession('id_ubicacion', post('id_ubicacion'));


									return json_encode(array(
										'estado' => 1,
										'succes' => 'Se ha actualizado el perfil',
									));
							...
							default:
								...
								$data = array(
									...
									'id_organizacion' => post('id_organizacion'),
									'id_rol' => post('id_rol'),
								);
